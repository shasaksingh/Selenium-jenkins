...
public class DriverScript {
	...
	public static String keyobject;
	public  By currentWebElement = null;
	static String actualLocator = null;
	...
	public static void main(String[] args) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, IOException, NoSuchMethodException, SecurityException {
		FileInputStream fs = new FileInputStream(System.getProperty("user.dir")+"//src//com//guru//config//config.properties");
		CONFIG= new Properties();
		CONFIG.load(fs);

		fs = new FileInputStream(System.getProperty("user.dir")+"//src//com//guru//config//or.properties");
		OR= new Properties();
		OR.load(fs);
	...
	public void start() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
		...
		suiteXLS = new Xls_Reader(System.getProperty("user.dir")+"//src//com//guru//excelfiles//Suite.xlsx");


		File f = new File("suiteXLS");
		if (f.exists())
			try {
				// Open the stream.
		...
		for(currentSuiteID=2;currentSuiteID<=suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET);currentSuiteID++){
			...
			if(suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equals(Constants.RUNMODE_YES)){
				...
				currentTestSuiteXLS=new Xls_Reader(System.getProperty("user.dir")+"//src//com//guru//excelfiles//"+currentTestSuite+".xlsx");
			...
			if(currentTestCaseName.equals(currentTestSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, Constants.TCID, currentTestStepID))){
				...
				for(int i=0;i<method.length;i++){
					...
					if(method[i].getName().equals(currentKeyword)){
						...
						keyobject= OR.getProperty(object);
						if(keyobject!=null)
						{
							actualLocator = extractActualLocator(keyobject);
							currentWebElement = extractWebElement(keyobject);
						}
						
						keyword_execution_result=(String)method[i].invoke(keywords,currentWebElement,data);
						APP_LOGS.debug(keyword_execution_result);
						resultSet.add(keyword_execution_result);
						// capture screenshot
						capturescreenShot_method.invoke(keywords,
								currentTestSuite+"_"+currentTestCaseName+"_TS"+(currentTestStepID-1)+"_"+(currentTestDataSetID-1),
	...
	public By extractWebElement(String keyobject){
		if(keyobject.startsWith("name!")){
			currentWebElement = By.name(actualLocator);
		}else if(keyobject.startsWith("xpath!")){
			currentWebElement =By.xpath(actualLocator);
		}else if(keyobject.startsWith("linkText!")){
			currentWebElement =By.linkText(actualLocator);
		}else if(keyobject.startsWith("id!")){
			currentWebElement =By.id(actualLocator);
		}else if(keyobject.startsWith("cssSelector!")){
			currentWebElement =By.cssSelector(actualLocator);
		}else if(keyobject.startsWith("tagName!")){
	...
	public String extractActualLocator(String locatorInSheet ){
		int underScoreIndex = locatorInSheet.indexOf('!');
